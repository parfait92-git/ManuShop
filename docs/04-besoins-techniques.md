...
**BF-68, attribution manuelle — page Super Admin (pas encore construite)** :
- Page protégée par l'appartenance à `platformAdmins` (pas juste `role === 'admin'`).
...
- **Expiration automatique — point d'architecture à trancher avant de coder** : il faut qu'"un serveur" repasse `role` à `client` une fois `subscriptionExpiresAt` dépassé, sans intervention humaine. L'architecture actuelle est 100% client (pas de `firebase-admin`, aucune tâche planifiée) ; les règles Firestore ne peuvent pas déclencher elles-mêmes une écriture. Deux options, aucune encore choisie :
  1. **Vercel Cron** (le projet est déjà déployé sur Vercel) déclenchant une route `/api/cron/expire-subscriptions`, qui utiliserait `firebase-admin` avec une clé de compte de service (nouveau secret à ajouter, jamais exposé au client) pour écrire en contournant les règles — c'est la seule façon d'agir "en tant que système" plutôt qu'en tant qu'un utilisateur authentifié précis.
  2. Firebase Cloud Functions avec un déclencheur planifié — écarté jusqu'ici par choix (le projet évite `firebase-admin`/l'infrastructure Firebase Functions), mais c'est l'outil natif pour ce genre de tâche.
  Dans les deux cas, c'est la première fois que le projet aurait besoin d'un accès Firestore privilégié côté serveur — à confirmer avec l'utilisateur avant de l'implémenter.
