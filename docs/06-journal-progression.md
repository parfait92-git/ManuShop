...
### 2026-09-25 — Spécification fonctionnelle complète Client/Commerçant/Super Admin, documentation seulement (aucun code)

L'utilisateur a fourni une spécification fonctionnelle détaillée couvrant les trois niveaux de privilège (client, commerçant, Super Admin), en demandant d'abord un avis puis l'ajustement des `.md`. Avant d'écrire quoi que ce soit, deux points de friction réels avec le code déjà livré ont été identifiés et remontés :
- La spec suppose qu'un commerçant peut posséder **plusieurs boutiques**, chacune avec son **propre abonnement** — alors que `adminSource`/`subscriptionPlan`/`subscriptionExpiresAt` vivent aujourd'hui sur `User` (posé le 2026-09-21, exploité par `PlatformAdminService` le 2026-09-23), sous l'hypothèse implicite d'une boutique par admin. Confirmé avec l'utilisateur : c'est une vraie migration à faire (déplacer ces champs vers `Shop`), pas une erreur d'interprétation de ma part.
- Le jeu de statuts de commande de la spec (`En cours d'analyse/Prêt pour la livraison/.../Retourné/Défectueux`) diffère de `OrderStatus` déjà défini (`pending/confirmed/delivering/delivered/cancelled`, Module 4, jamais encore utilisé par une UI) — à remplacer, pas à faire cohabiter.

**Deux questions posées avant d'écrire les docs** (la spec ne les tranchait pas) : le rôle `seller` (employé invité, déjà construit via `TeamList`/`InviteSellerForm`) reste-t-il à côté du nouveau vocabulaire "commerçant" ? → **oui, conservé**. Qui définit les tags colorés de catégorie pour la page Marché ? → **liste fermée gérée exclusivement par le Super Admin**, pas ouverte aux commerçants.

**Docs mis à jour** (aucun fichier `src/` touché — cette entrée est purement documentaire) :
- `02-besoins-fonctionnels.md` — 10 nouveaux modules (13→22), BF-71 à BF-119 (49 nouveaux besoins) : fiche produit/avis, commande/retour client, assistant de création de boutique multi-étapes, espace commerçant étendu (multi-boutique, publication produit distincte de la suppression), statuts de commande + corbeille générique, tableau de bord/factures avancées, paramètres marchand premium, page Marché + tags système, messagerie commerçant↔Super Admin, supervision Super Admin.
- `04-besoins-techniques.md` — nouveau §12 : migration `User`→`Shop` pour l'abonnement (avec la liste précise du code déjà livré qu'elle impacte), `Product.isPublished`, `OrderStatus` révisé, convention de corbeille générique (`deletedAt` + `TrashService`), nouveaux modèles (`Review`, `CategoryTag`, `Message`, `ShopVisitEvent`, `ActivityLogEntry`), note sur le paiement (interface seulement, prestataire choisi plus tard).
- `05-plan-de-travail.md` — nouvelle Phase 1ter, séquencée pour poser d'abord ce dont le reste dépend (migration de modèle, puis corbeille, avant les fonctionnalités qui s'appuient dessus).
- `01-business-plan.md` — §10.1 : précision que toute visibilité publique (boutique, produit) devient une fonctionnalité payante, et que le modèle multi-boutique est désormais un besoin concret plutôt qu'une préparation pour plus tard.
- `03-besoins-non-fonctionnels.md` — BNF-35 mis à jour (multi-boutique concret), BNF-16bis ajouté (validation des liens réseaux sociaux avant enregistrement, BF-106).

**Explicitement pas fait** : aucune implémentation de ces 49 nouveaux besoins — l'utilisateur a demandé l'ajustement de la documentation, pas le code. La migration `User`→`Shop` pour l'abonnement (§12.1 de 04-besoins-techniques.md) est la prochaine étape technique logique, puisque tout le reste de la Phase 1ter en dépend. Rien à vérifier via `npm run lint`/`test`/`build` cette fois — uniquement des fichiers `.md` modifiés. Rien de commité.

### 2026-09-25 — Fiche produit & paiement (réels) ; suivi de commande/avis/retour reportés au Module 4

Suite du 2026-09-25 précédent : un générateur de design IA a produit les 5 écrans du parcours client (Module 13/14). Avant de coder, vérifié qu'aucun code n'existe pour les commandes (`OrderService`/`OrderRepository` inexistants, "Commander" ouvre juste un lien WhatsApp sans jamais écrire dans Firestore) ni pour les avis (aucun modèle `Review`). Décidé avec l'utilisateur : construire pour de vrai uniquement la fiche produit (BF-71/72/73) et l'écran de paiement (BF-78, déjà documenté comme interface seule) ; suivi de commande, avis après livraison et demande de retour (BF-75/76/77) reportés — les construire maintenant aurait donné des pages qui ont l'air de marcher mais ne peuvent jamais afficher de vraie donnée.

**Fiche produit** (`/catalogue/[productId]`, `ProductDetailPageContent.tsx`) : données réelles via `productService.getProduct(id)` (déjà existant). Nouveau modèle `Review` (lecture seule cette tranche — la soumission est BF-76, reportée) : `src/models/review/Review.ts`, `IReviewRepository`/`ReviewRepository` (calqués sur `CategoryRepository`), `ReviewService` avec `getAverageRating()` qui renvoie `null` plutôt qu'une note fictive quand il n'y a aucun avis. `firestore.rules` : nouveau bloc `reviews` (lecture publique, écriture verrouillée tant que BF-76 n'existe pas).

**`StorefrontProductCard.tsx`** restructuré pour lier vers la fiche produit sans imbriquer le bouton favoris (cœur) dans le nouveau `<Link>` (un `<button>` dans un `<a>` est invalide en HTML) — le cœur sort du lien et reste positionné via `<article>` devenu `relative`.

**Écran de paiement** (`/checkout/payment`, `PaymentMethodPageContent.tsx`) : Visa/Orange Money/MTN Mobile Money, champ conditionnel, protégé par `ProtectedRoute` (BF-74, authentification de base uniquement — pas la nuance "compte anonyme doit compléter son profil"). Accessible depuis `CartPanel` via un nouveau bouton "Choisir un mode de paiement", à côté du bouton WhatsApp existant qu'il ne remplace pas. "Payer" ne simule jamais un succès : `sonner` (dépendance installée depuis le début du projet mais jamais montée nulle part — `<Toaster/>` ajouté au layout racine) affiche honnêtement que ce n'est pas encore disponible.

**Bug trouvé et corrigé en testant en conditions réelles** : `Promise.all([productService.getProduct(id), reviewService.listByProduct(id)])` faisait échouer TOUTE la page si la requête `reviews` était rejetée (ex. `firestore.rules` pas encore déployées en console — confirmé exactement ce cas en testant contre le vrai projet Firebase) — la page restait bloquée sur "Chargement..." indéfiniment même si le produit lui-même existait. Corrigé en découplant les deux requêtes : un avis qui ne charge pas ne doit jamais empêcher le produit de s'afficher. Test de non-régression ajouté.

**Deuxième bug trouvé en testant visuellement** : les deux nouvelles pages avaient été stylées en thème sombre (`text-white`, `bg-white/5`...), en supposant à tort que tout le storefront suivait le thème sombre glassmorphism de la landing page. Capture à l'appui : `/catalogue` et `StorefrontHeader` sont en réalité en thème clair neutre (tokens shadcn partagés, comme le dashboard). Corrigé — les deux pages utilisent maintenant `text-muted-foreground`/`border-border`/etc., cohérent avec `CataloguePageContent`. Le style "sombre" du primer `design-prompts.txt` ne s'applique donc qu'à la landing page marketing (`/`), pas au reste du storefront — à corriger dans ce fichier à l'occasion.

Tests ajoutés : `ReviewService.test.ts`, `ProductDetailPageContent.test.tsx` (7 tests, dont la non-régression Promise.all), `PaymentMethodPageContent.test.tsx`, `StorefrontProductCard.test.tsx` (nouveau, aucun n'existait avant).

Vérifié : `npm run lint`, `npx tsc --noEmit`, `npm run build` (25 routes, +2 — `/catalogue/[productId]`, `/checkout/payment`) et `npm run test:coverage` (209 tests, +17, aucune régression). Vérifié aussi en conditions réelles (Playwright contre le serveur dev + le vrai projet Firebase) : état "Produit introuvable" et redirection `/erreur?code=401` pour un visiteur non connecté sur `/checkout/payment`, les deux confirmés visuellement après correction du thème. Rien de commité.

### 2026-09-25 — Assistant "Créer ma boutique" (BF-79→85) : migration d'abonnement `User`→`Shop`, Server Action, bug de soumission accidentelle trouvé en testant en direct

Suite des captures reçues pour le Module 15 (items 6-9 de `design-prompts.txt`, assistant en 4 étapes : Infos/Logo/Récapitulatif/Abonnement). Avant de construire, exploration des règles Firestore a révélé un vrai blocage : un `client` ne peut **littéralement pas** devenir `admin` en créant une boutique aujourd'hui — `shops.create` exige déjà `role == 'admin'`, et `users.update` interdit à un compte de changer son propre `role`, quel que soit le contexte. Remonté à l'utilisateur avec les options possibles ; réponse explicite : tout construire pour de vrai, migration comprise.

**Migration de modèle** (04-besoins-techniques.md §12.1, révisée en cours d'implémentation par rapport à l'ébauche initiale) : `subscriptionPlan`/`subscriptionExpiresAt` déplacés de `User` vers `Shop` — un abonnement finance une boutique précise (BF-83), pas le compte (BF-85, multi-boutique). Suppression sans risque : confirmé par grep qu'aucun code livré n'écrivait jamais ces deux champs (BF-69 n'avait jamais été construit). `User.adminSource` **conservé**, contrairement à ce que l'ébauche initiale du plan proposait — toujours utile pour BF-68 (attribution manuelle par le Super Admin, geste au niveau du compte, indépendant de toute boutique) ; `Shop` gagne son propre `adminSource` (sens différent : comment *cette boutique* a été créée) plus `sector?` (nouveau champ réel, jusque-là seulement dans `MockShop`).

**Nouvelle Server Action** `createShopAction` (`src/server/actions/shopActions.ts`) : même schéma que l'attribution manuelle du Super Admin (BF-68) — `firebase-admin`, contourne les règles Firestore côté serveur après vérification d'identité (`requireCaller`, pas de privilège particulier requis : n'importe quel compte connecté peut créer une boutique). Crée le doc `shops` (`adminSource: "subscription"`, plan + date d'expiration calculée) puis met à jour `users/{uid}` (`shopId` toujours ; `role`/`adminSource` seulement si le compte n'était pas déjà admin — un admin qui crée une 2ᵉ boutique garde son `adminSource` d'origine).

**`CreateShopWizard.tsx`** : un seul `useForm` sur les 4 étapes, `trigger()` par étape avant "Suivant", recadrage de logo réutilisant `ImageCropDialog`/`cropImageToSquare` (nouveau `uploadShopLogo`, dossier Cloudinary `manushop/shops` — `/api/uploads` accepte désormais un `folder` en liste blanche). Récapitulatif honnête : "Non renseigné" pour tout champ optionnel resté vide, jamais de valeur fabriquée (BF-82). Fermer/annuler réinitialise tout (BF-84) — sûr puisqu'aucune écriture n'a lieu avant la confirmation finale. Entrée "Créer ma boutique" ajoutée au menu compte (`StorefrontHeader`), visible uniquement pour `role === 'client'`.

**Bug réel trouvé en vérifiant en direct** (Playwright contre le serveur dev + vrai Firebase, compte anonyme de test) : cliquer sur "Continuer vers l'abonnement" (étape 3→4) déclenchait **immédiatement** une vraie tentative de création de boutique, sans jamais laisser voir l'étape Abonnement — confirmé côté serveur (log dev : `createShopAction` appelée, n'a échoué que parce que `FIREBASE_SERVICE_ACCOUNT_KEY_BASE64` est absent en local ; en production l'écriture aurait réussi avec le plan par défaut, jamais choisi par l'utilisateur). Cause : le bouton "Suivant/Continuer" (`type="button"`) et le bouton final "Confirmer" (`type="submit"`) occupaient la même position dans un ternaire sans `key` distincte — React réutilise alors le même nœud DOM et se contente de changer son attribut `type` en place pendant le clic qui fait avancer l'étape. Le navigateur, en traitant l'action par défaut du clic *après* que React a déjà muté cet attribut, voit `type="submit"` et soumet le formulaire tout seul. Corrigé en donnant à chaque branche du ternaire une `key` distincte (`"next"`/`"submit"`), forçant React à démonter/remonter plutôt qu'à muter en place. Reproduit avant correctif, confirmé absent après (nouvelle passe Playwright : le bouton reste "Confirmer et créer ma boutique", inactif, aucun nouvel appel serveur).

**Prix des abonnements** (`src/lib/subscriptionPlans.ts`) repris tels quels de la maquette générée par l'outil de design (500/2 500/8 000/20 000/60 000 FCFA) — **placeholders, pas une décision business validée**, à confirmer avant tout lancement réel.

Tests ajoutés : `subscriptionPlans.test.ts`, `shopActions.test.ts`, `CreateShopWizard.test.tsx` (7 tests), `upload.test.ts`, extension de `StorefrontHeader.test.tsx`/`AuthService.test.ts`.

Vérifié : `npm run lint`, `npx tsc --noEmit`, `npm run build` (toujours 25 routes) et `npm run test:coverage` (232 tests, +23, aucune régression) — le tout **après** le correctif du bug de soumission accidentelle. Vérification visuelle complète des 4 étapes en conditions réelles (Playwright, compte anonyme + onboarding réel, thème clair confirmé cohérent avec le reste du storefront), volontairement arrêtée avant le clic "Confirmer" final pour ne pas écrire une vraie boutique dans le projet Firebase de l'utilisateur. Rien de commité.

### 2026-09-25 — Reprise de session : corbeille/gestion multi-boutique/journal d'activité (Phase 1ter §1-2) livrés sans être journalisés, `npm run lint` cassé au point d'arrêt

Cette session a repris un état où une quantité de travail conséquente (multi-boutique BF-87, corbeille générique BF-99/100, journal d'activité BF-98, bandeau de publication BF-88) avait déjà été codée dans la session précédente mais **jamais consignée dans ce journal** ni commitée — seul indice disponible : l'entrée précédente (assistant de création de boutique) et l'état réel du répertoire de travail (`git status`) ne correspondaient plus. `npm run lint` échouait avec 2 erreurs dans `TrashPageContent.tsx` (`react-hooks/set-state-in-effect` — le même piège que celui documenté sur `ScrollReveal.tsx` le 2026-09-20 : `setState` synchrone dans le corps d'un effet), ce qui situe précisément le point d'arrêt.

**Corrigé, avec les mêmes patrons déjà établis dans le projet** (`queueMicrotask` pour un reset d'état synchrone, `.then()` + drapeau `active` plutôt qu'une fonction `async` appelée directement dans l'effet — voir `useCurrentShop.ts`/`CataloguePageContent.tsx`) :
- `CountdownDialog` (compte à rebours de suppression définitive) : `setSecondsLeft(COUNTDOWN_SECONDS)` déplacé dans un `queueMicrotask`.
- `TrashPageContent` : chargement initial (`load()`) réécrit en `Promise.all(...).then(...)` avec drapeau `active`, au lieu d'appeler une fonction `async` locale directement dans l'effet.
- Nettoyage au passage : dépendance `onConfirm` ajoutée à la place du commentaire `eslint-disable` (devenu inutile après le refactor, l'avertissement du directive inutilisé aurait sinon persisté).

**Audité l'état du reste du travail non journalisé** (tout compile, aucune autre erreur) : `ShopManagementPageContent` (BF-87, liste des boutiques du commerçant + bascule `profile.shopId` via `AuthService.switchShop` + relance de `CreateShopWizard` pour une 2ᵉ boutique), `TrashService`/`ActivityLogService`/`ActivityLogRepository` (+ modèle `ActivityLogEntry`, BF-98) et `PublicationBanner` (BF-88, bandeau persistant dans `dashboard/layout.tsx` tant que `shop.isPublished` est faux) sont tous câblés et fonctionnels — `activityLogService.log*` est déjà appelé depuis `ProductList`/`CategoryManager`/`TrashPageContent`/`ShopSettingsForm`, et `firestore.rules` a déjà son bloc `activityLog` (lecture/écriture scopées par boutique, `update`/`delete` interdits).

**Trouvé et corrigé** : `DashboardSidebar.tsx` (déjà modifié dans la session précédente) pointait vers `/dashboard/activity` ("Journal d'activité") — lien mort, aucune page ni composant ne consommait `activityLogService.listRecent()`. Contraire au principe déjà établi le 2026-09-21 ("chaque bouton/lien doit mener à une vraie page"). Ajouté :
- `ActivityLogPageContent.tsx` (nouveau) : liste triée par date décroissante côté client (le repository ne trie pas, même choix que le tri des lignes de `TrashPageContent`), une icône par `targetType` (produit/catégorie/boutique), un libellé humain par `ActivityLogAction` dérivé de `metadata.productName`/`categoryName` quand disponible.
- `src/app/dashboard/activity/page.tsx` : même patron que `/dashboard/trash` (`ProtectedRoute allowedRoles={["admin", "seller"]}`).
- Tests : `ActivityLogPageContent.test.tsx` (état vide, libellés par action, tri par date).

Vérifié : `npm run lint`, `npx tsc --noEmit`, `npm run build` (28 routes, +1 — `/dashboard/activity`) et `npm run test:coverage` (268 tests, +3, aucune régression). Pas de vérification visuelle Playwright cette fois (page purement lecture seule, pas d'interaction risquée). Rien de commité — tout l'historique ci-dessus (multi-boutique, corbeille, journal d'activité, bandeau de publication) reste non commité comme le reste du projet.

**Audit complémentaire du reste de la Phase 1ter** (`05-plan-de-travail.md` mis à jour en conséquence) : §3 (BF-89/90) est en fait déjà livré aussi — `Product.isPublished` (`ProductService.setPublished`/`isVisibleToCustomers`) existe et `/catalogue` filtre déjà dessus ; seule la collection `CategoryTag` (Super Admin) manque encore. §1 et §2 (gestion multi-boutique, corbeille) confirmés complets par la même occasion (voir plus haut).

**Prochaine étape suggérée (Phase 1ter, voir `05-plan-de-travail.md`)** : §3 — collection `CategoryTag` (Super Admin uniquement) + `Category.tagId`, seul chaînon manquant avant de généraliser `/demo-catalogue` en vraie page Marché (§7) ; ou Module 3 (Stock, §4), toujours pas commencé et prioritaire depuis plusieurs sessions.

### 2026-09-25 — Bug signalé : impossible de se reconnecter avec un autre compte Google/Facebook après déconnexion

Signalé par l'utilisateur : une fois authentifié via Google ou Facebook, après déconnexion (`logout()`), impossible de se réauthentifier avec un **autre** compte sur le même navigateur — la reconnexion se refait automatiquement avec le même compte.

**Cause** : `AuthService.loginWithGoogle()`/`loginWithFacebook()` (`src/services/AuthService.ts`) appelaient `signInWithPopup` avec un provider par défaut, sans paramètre personnalisé. Firebase réutilise alors silencieusement la session Google/Facebook encore active dans le navigateur (cookie du fournisseur, indépendant de la session Firebase que `logout()` ferme) — la popup ne propose jamais de sélecteur de compte, elle reconnecte directement au dernier compte utilisé. `signOut(auth)` ferme uniquement la session **Firebase**, ce qui est le comportement correct — le bug est côté paramétrage du provider OAuth, pas côté déconnexion.

**Corrigé :** `provider.setCustomParameters({ prompt: "select_account" })` pour Google (force l'affichage du sélecteur de compte à chaque connexion) et `provider.setCustomParameters({ auth_type: "reauthenticate" })` pour Facebook (force le ré-affichage du dialogue de connexion même si le navigateur est déjà connecté à Facebook).

Tests : `AuthService.test.ts` étendu (mocks `GoogleAuthProvider`/`FacebookAuthProvider` dotés de `setCustomParameters`, assertions sur les paramètres passés).

Vérifié : `npm run lint`, `npx tsc --noEmit` et `npm run test:coverage` (268 tests, aucune régression). Pas de vérification Playwright possible (nécessiterait de vrais comptes Google/Facebook multiples) — comportement déduit avec certitude de la documentation Firebase sur `signInWithPopup`/`setCustomParameters`, correctif standard pour ce problème connu. Rien de commité.

### 2026-09-25 — Champ `role: 'super-admin'` ajouté à `PlatformAdmin` (informatif)

Demande : que les documents `platformAdmins` portent `role: 'super-admin'`, en cohérence avec `role: 'admin'`/`'seller'` sur `User`. **Friction remontée avant de coder** : le projet documente très explicitement, à plusieurs endroits (`PlatformAdmin.ts`, `firestore.rules`, `04-besoins-techniques.md` §11.5), que le Super Admin n'est délibérément **pas** un rôle — seule l'appartenance à la collection `platformAdmins` fait foi, précisément pour qu'aucun chemin de l'app ne puisse jamais accorder ce privilège via la valeur d'un champ. Question posée à l'utilisateur : le champ doit-il rester purement informatif, ou devenir une donnée réellement vérifiée par la logique d'autorisation ? Réponse : **informatif seulement**.

**Fait :** `role: 'super-admin'` (type littéral) ajouté à l'interface `PlatformAdmin` (`src/models/platform/PlatformAdmin.ts`), avec un commentaire explicite que l'autorisation ne le lit jamais — elle reste basée sur `exists(platformAdmins/{email})`, inchangé dans `firestore.rules` et dans tout le code (`PlatformAdminRepository.exists()`, jamais de lecture du contenu du document). Aucun chemin de l'app n'écrit dans cette collection (toujours `allow write: if false`), donc ce champ n'a d'effet que sur les documents créés **manuellement** depuis la console Firebase — `04-besoins-techniques.md` §11.5 mis à jour pour le signaler à l'utilisateur lors de la prochaine création manuelle d'un Super Admin.

**Non fait, volontairement** : aucun changement à `firestore.rules`, `UserRole` (`'admin' | 'seller' | 'client'`, inchangé), ni à la logique de `PlatformAdminService`/`SuperAdminRoute`/`AuthProvider` — tous continuent de vérifier uniquement l'existence du document, jamais son contenu.

Vérifié : `npm run lint`, `npx tsc --noEmit` et `npm run test:coverage` (268 tests, aucune régression — aucun test ne construisait d'objet `PlatformAdmin` typé, rien à mettre à jour). Rien de commité.

### 2026-09-25 — Commit du travail accumulé + paramètres du compte personnel (BF-120) ; notification de version par email documentée mais reportée (BF-121)

**Commit** : sur demande explicite, tout le travail accumulé depuis `15bcf43` (fiche produit/paiement, assistant multi-boutique, corbeille, journal d'activité, correctifs auth, `PlatformAdmin.role`) a été commité en un seul commit (`24d0eae`, 93 fichiers). Étant donné l'ampleur, un futur nettoyage d'historique (rebase interactif en plusieurs commits thématiques) pourrait être envisagé avant un `push`, mais non fait ici — pas demandé.

**Demande suivante** : notifier tous les utilisateurs par email à chaque incrément de version, et donner au client des paramètres de profil/compte. **Friction remontée avant de coder** : le projet n'a aucune capacité d'envoi d'email (aucun fournisseur configuré), et une partie des comptes (téléphone/anonyme) n'ont pas d'email. Questions posées à l'utilisateur : source de la version (retenu : `package.json`), fournisseur d'email (retenu : **aucun pour l'instant**, report explicite de l'envoi réel), audience (retenu : tous les comptes avec un email). Résultat : construire uniquement la préférence de notification côté compte maintenant ; documenter le reste (§13 de `04-besoins-techniques.md`) comme un BF non commencé, sur le même modèle que BF-69 (abonnement payant, lui aussi bloqué sur un point ouvert et documenté plutôt que codé à moitié).

**Fait — BF-120, paramètres du compte personnel (`/mon-compte`)** :
- `User.notifyByEmail?: boolean` (absent = `true`, même convention que `Product.isPublished`) — préférence enregistrée mais pas encore consommée par un envoi réel (honnêteté du même type que `notifyOrdersByEmail` sur `ShopSettingsForm`, déjà enregistré avant que le Module 4 existe).
- `AuthService.updateProfile(uid, data)` (délègue à `IUserRepository.update`, déjà autorisé par les règles Firestore existantes — aucun changement de `firestore.rules` nécessaire, l'auto-modification de son propre profil hors `role`/`shopId` était déjà permise).
- `AccountSettingsSchema` (`src/lib/validation/auth.ts`) + `AccountSettingsForm` (`src/components/account/`, nouveau dossier — page personnelle distincte des dossiers `dashboard`/`storefront` existants) : nom, téléphone (`PhoneInput`), photo (même pipeline de recadrage carré que le logo boutique — `AVATAR_IMAGE_SIZE`, `uploadAvatar`, dossier Cloudinary `manushop/users` ajouté à la liste blanche de `/api/uploads`), toggle notifications par email, section lecture seule (identifiant de connexion, rôle, ancienneté), bouton "Changer le mot de passe" (réutilise `AuthService.sendPasswordReset`, affiché seulement pour les comptes email/mot de passe — vérifié via `firebaseUser.providerData`).
- **Email volontairement non éditable** : le modifier dans Firestore sans toucher à Firebase Auth désynchroniserait l'identifiant de connexion réel — nécessiterait un flux `updateEmail` + réauthentification, hors scope de cette tranche.
- Accessible depuis le menu "Mon compte" de `StorefrontHeader` (tout rôle) et depuis le menu du dashboard `DashboardTopbar` (nouveau lien, absent jusqu'ici) — personne n'avait de moyen d'éditer son propre profil auparavant, quel que soit le rôle.

**Reporté — BF-121, notification de nouvelle version par email** : documenté (§13 de `04-besoins-techniques.md`) avec les décisions déjà actées (source de version, audience) et les points ouverts (fournisseur d'email, mécanisme de déclenchement — piste par défaut : même schéma Vercel Cron + `CRON_SECRET` que le job d'expiration d'abonnement non construit, §11.5). Rien codé : construire une détection de version qui ne peut rien envoyer aurait été un pipeline à moitié fini.

Tests ajoutés : `AccountSettingsForm.test.tsx` (6 cas : chargement, pré-remplissage, sauvegarde + `refreshProfile`, toggle notification, reset mot de passe visible/masqué selon le provider), `AuthService.test.ts` (+1 cas `updateProfile`).

Vérifié : `npm run lint`, `npx tsc --noEmit`, `npm run build` (29 routes, +1 — `/mon-compte`) et `npm run test:coverage` (275 tests, +7, aucune régression). Pas de vérification Playwright cette fois (nécessiterait une session authentifiée réelle) — comportement couvert par les tests unitaires du formulaire. Rien de commité (ce commit-ci reste pour la prochaine demande explicite de l'utilisateur).

### 2026-09-25 — Retrait de l'authentification téléphone (SMS OTP) et anonyme

Demande explicite de l'utilisateur. Ces deux méthodes avaient été ajoutées le 2026-09-21 (refonte `/login`, voir entrée du journal). Retiré :

- `AuthService.ts` : suppression de `loginAnonymously`/`startPhoneSignIn`/`confirmPhoneCode` et des imports Firebase Auth désormais inutilisés (`signInAnonymously`, `signInWithPhoneNumber`, `ConfirmationResult`, `RecaptchaVerifier`).
- `src/lib/validation/auth.ts` : suppression de `PhoneLoginSchema`/`PhoneCodeSchema`.
- `LoginForm.tsx` : suppression des onglets Email/Téléphone (ne reste que le formulaire email — plus besoin de bascule), de `PhoneLoginForm` (formulaire numéro + code SMS) en entier, du bouton "Anonyme" (grille 3→2 colonnes pour Google/Facebook), de l'ancre `#recaptcha-container` (`RecaptchaVerifier` n'existe plus), et du cas d'erreur `auth/invalid-verification-code` (devenu inatteignable).

**Volontairement pas touché** : `PhoneInput`, `User.phone`, `Shop.phone`/`Shop.whatsapp` — ce sont des numéros de **contact** (profil personnel, coordonnées boutique), sans rapport avec l'authentification, toujours utilisés partout ailleurs (`/mon-compte`, `ShopSettingsForm`, etc.). `User.email` reste optionnel (défensif pour un cas limite Facebook où la permission email est refusée) plutôt que redevenu obligatoire.

**Conséquence assumée, pas de migration** : les comptes déjà créés par téléphone ou anonymement avant ce retrait (s'il en existe en production) perdent tout moyen de se reconnecter — aucune tentative de bascule vers un autre mode de connexion pour eux.

Commentaires mis à jour en cascade pour ne plus référencer ces flux retirés : `User.ts`, `ProtectedRoute.tsx`, `GuestRoute.tsx`, `PlatformAdminService.ts`, `requireSuperAdmin.ts`, `mockData.ts`, `checkout/payment/page.tsx`. Documentation : `02-besoins-fonctionnels.md` (révision ajoutée sous le paragraphe du 2026-09-21, BF-74 mis à jour — la nuance "compte anonyme doit compléter son profil" n'a plus lieu d'être), `04-besoins-techniques.md` (§14, + correction de l'audience BF-121 rédigée juste avant dans la même session).

Tests : `AuthService.test.ts` (suppression des cas `loginAnonymously`/`startPhoneSignIn`/`confirmPhoneCode` et des mocks associés), `auth.test.ts` (suppression des cas `PhoneLoginSchema`/`PhoneCodeSchema`). Aucun test n'existait pour `LoginForm.tsx` lui-même (rien à mettre à jour côté composant).

Vérifié : `npm run lint`, `npx tsc --noEmit`, `npm run build` (toujours 29 routes) et `npm run test:coverage` (268 tests, -7, aucune régression). **Pas de vérification visuelle** cette fois — aucun outil de navigateur (Playwright) disponible dans cette session, contrairement aux sessions précédentes. Rien de commité.

### 2026-09-25 — Notification de nouvelle commande vers WhatsApp Business (Meta Cloud API)

Demande : brancher une notification de commande vers WhatsApp Business, Facebook, Instagram ou TikTok (API Meta). **Friction remontée avant de coder** : sur ces quatre canaux, seul WhatsApp Business Cloud API convient à un envoi transactionnel côté serveur — Messenger (Facebook) et Instagram Messaging exigent une fenêtre de conversation ouverte par le destinataire ou un tag approuvé, et TikTok n'est pas un produit Meta et n'a pas d'équivalent. Confirmé avec l'utilisateur : WhatsApp seulement pour cette notification ; Facebook/Instagram/TikTok restent réservés à la **publication** de contenu (Module 8, non commencé), pas à la notification de commande. Question posée : quel numéro reçoit l'alerte ? Réponse : réutiliser `Shop.whatsapp` (déjà affiché aux clients), plutôt qu'un nouveau champ dédié.

**Fait** : `src/lib/whatsappBusiness.ts` (`sendOrderNotification`) — appelle l'API Cloud de Meta avec un message **template** (obligatoire pour un envoi business-initiated hors fenêtre de 24h ; texte libre impossible ici). Nom de template configurable (`WHATSAPP_ORDER_TEMPLATE_NAME`, sinon `new_order_notification` par défaut) ; jamais d'exception levée — identifiants manquants, numéro invalide ou échec réseau/API sont logués et avalés, pour ne jamais faire échouer une création de commande à cause d'une notification. Branché dans `createOrderAction` (`src/server/actions/orderActions.ts`) juste après le commit de la commande, conditionné à `Shop.notifyOrdersBySocial !== false` — préférence déjà présente sur `ShopSettingsForm` depuis une session précédente mais sans effet réel jusqu'ici, elle pilote maintenant un vrai envoi. `.env.example` et `04-besoins-techniques.md` (§8, nouveau §15) mis à jour.

**Point ouvert, action manuelle requise** : aucun identifiant Meta/WhatsApp n'est renseigné dans `.env.local` (`META_*`/`WHATSAPP_*` tous vides) et aucun template n'existe encore côté Meta Business Manager — le code est prêt mais n'enverra rien tant que l'utilisateur n'a pas créé le compte WhatsApp Business, fait approuver un template et rempli ces variables.

Tests : `whatsappBusiness.test.ts` (nouveau, 7 cas — identifiants absents, envoi réussi, template personnalisé, échec API, échec réseau, numéro invalide), `orderActions.test.ts` (+2 cas — notification envoyée à la création, ignorée si la boutique a désactivé `notifyOrdersBySocial`).

Vérifié : `npx tsc --noEmit`, `npm run lint`, et la suite de tests ciblée (`orderActions.test.ts` + `whatsappBusiness.test.ts`, 20 tests, tous passants). `npm run test:coverage` complet : 6 échecs pré-existants dans `PaymentMethodPageContent.test.tsx`/`phone-input.test.tsx` (erreur d'import Node de `@firebase/auth`), confirmés sans rapport avec ce changement en isolant les nouveaux fichiers via `git stash`. Rien de commité.

### 2026-09-25 — Module 4 (Commandes) construit en entier — collision avec une session parallèle, reprise après coordination

Demande : "gère déjà la fonctionnalité des commandes", avant que l'utilisateur ne vide les données réelles pour repartir sur des tests propres. Le modèle `Order`/`OrderStatus`/`OrderItem` existait depuis Phase 0 mais n'était branché nulle part (confirmé par grep : aucun repository/service, "Commander" n'écrivait jamais dans Firestore). **Décisions prises avec l'utilisateur avant de coder** (plusieurs points explicitement "pas encore tranchés" dans les docs) :
- Le bouton "Payer" de `/checkout/payment` devient "Confirmer ma commande" : écrit une vraie commande, paiement à la livraison en attendant une intégration réelle (BF-78 reste hors scope).
- Annulation (BF-23) : client **ou** commerçant, uniquement tant que `under_review` (avant expédition), motif obligatoire.
- Stock : décrément/incrément automatique de `Product.stock` à la commande/annulation/retour — mécanique minimale, pas le Module 3 (Stock) en entier, qui reste non construit.
- Notification automatique vers le réseau social principal "en arrière-plan" (demandée par l'utilisateur) : impossible sans API officielle (WhatsApp Business Cloud API/Meta Graph/TikTok, comptes développeurs non créés, Module 8) — retenu : documenter comme bloqué, construire uniquement une notification **interne** réelle (cloche du tableau de bord) à la place. Une session parallèle a ensuite construit l'intégration WhatsApp Business réelle malgré tout (voir entrée juste au-dessus) — les deux se sont avérées compatibles.

**Collision avec une session parallèle (`manushop-c7`)** : en cours de route, `orderActions.ts`/`orderActions.test.ts` ont changé sur le disque sans que j'y touche — une autre session Claude Code, ouverte par l'utilisateur dans une autre fenêtre, étendait les mêmes fichiers pour y brancher `whatsappBusiness.ts`. Signalé immédiatement à l'utilisateur plutôt que de continuer à écrire par-dessus ; réponse : arrêter cette session et laisser l'autre terminer. Une fois cette session redevenue *idle*, l'utilisateur a redemandé de reprendre — vérifié que l'ajout de l'autre session (notification WhatsApp, `.env.example`, `04-besoins-techniques.md` §15) était additif et compatible (signatures de `createOrderAction`/`updateOrderStatusAction` inchangées), puis complété le reste.

**Fait par cette session :**
- `src/models/order/OrderStatus.ts` réécrit avec le vocabulaire BF-95 (`under_review/ready_for_delivery/delivering/delivered/returned/defective`) + `cancelled` (BF-23, hors périmètre initial de BF-95, clarifié avec l'utilisateur) ; `Order.clientId` devient optionnel (absent = commande manuelle BF-21), `cancelReason`/`returnReason`/`restockedAt` ajoutés.
- `IOrderRepository`/`OrderRepository` — **lecture seule** délibérément : `getById`/`listByShop`/`listByClient` seulement, aucune mutation. Les écritures passent par `src/server/actions/orderActions.ts` (`createOrderAction`/`updateOrderStatusAction`, `firebase-admin`, `db.batch()`) pour rester atomiques avec l'ajustement de stock et revalider l'autorisation en code (client propriétaire vs commerçant de la boutique) — même schéma que `createShopAction`/`platformAdminActions`. `firestore.rules` verrouille `orders` en écriture (`allow write: if false`), lecture scopée par `clientId` ou par boutique (`role in ['admin','seller']` + `shopId`).
- `OrderService` (façade côté client, résout son propre `idToken` comme `PlatformAdminService`) : `listByShop`/`listByClient`/`getOrder` (lecture directe), `createOrder`/`advanceStatus`/`cancelOrder`/`markReturned` (Server Actions).
- `ActivityLogService` étendu (`order.created`/`order.status_changed`/`order.cancelled`/`order.returned`) — jamais appelé pour la création d'une commande **client** (les règles interdisent à un `role: 'client'` d'écrire dans `activityLog`), seulement pour les actions commerçant.
- **Tableau de bord** : `/dashboard/orders` (`OrdersPageContent`, remplace le `ComingSoonCard`) — table filtrable par statut (dont un lien direct `?status=under_review` depuis la cloche), actions de progression par statut, `OrderReasonDialog` (motif obligatoire, partagé avec le suivi client) pour annulation/retour/défectueux, `ManualOrderDialog` (BF-21, sélection de produits du propre catalogue + quantités). Cloche de notification (`DashboardTopbar`) rendue réelle via `useNewOrdersCount` (seul usage de `onSnapshot` du projet — justifié ici précisément parce que l'utilisateur a demandé une notification "directe"). `DashboardHomeContent` : cartes "Ventes du mois"/"Commandes du mois"/"Nouveaux clients" et "Ventes récentes" passées de "Bientôt" à de vraies données.
- **Client** : `/checkout/payment` (`PaymentMethodPageContent`) réécrit — collecte nom/téléphone/adresse (pré-remplis depuis le profil), "Confirmer ma commande" crée la commande, vide le panier, redirige vers `/mes-commandes`. Nouvelle page `/mes-commandes` (`MyOrdersPageContent`, BF-75) : liste des commandes du client, bouton Annuler tant que `under_review`. Lien "Mes commandes" ajouté au menu compte de `StorefrontHeader`.
- Le bouton "Commander via WhatsApp" du panier (`CartPanel`) est resté inchangé — canal informel en plus de la vraie commande, pas un remplacement.

Tests ajoutés (au-delà de ceux déjà comptés par la session parallèle) : `OrderService.test.ts`, `OrdersPageContent.test.tsx` (6 cas), `MyOrdersPageContent.test.tsx` (4 cas), `ManualOrderDialog.test.tsx` (5 cas), `OrderReasonDialog.test.tsx` (3 cas), `useNewOrdersCount.test.ts` (3 cas, 100% de couverture), `ActivityLogService.test.ts` (+4 cas). `PaymentMethodPageContent.test.tsx` entièrement réécrit (l'ancien testait l'ex-écran factice). Au passage : `PlatformAdminService.test.ts` avait un intitulé de test résiduel mentionnant "phone/anonymous accounts" (retrait du 2026-09-25 précédent) — corrigé.

Vérifié : `npm run lint`, `npx tsc --noEmit`, `npm run build` (30 routes, +1 — `/mes-commandes`) et `npm run test:coverage` (322 tests, aucune régression — les "6 échecs pré-existants" que l'autre session avait signalés étaient bien dans `PaymentMethodPageContent.test.tsx`, réécrit ici ; `phone-input.test.tsx` s'est avéré être un faux positif, passant systématiquement en isolation). Pas de vérification Playwright (aucun outil de navigateur disponible dans cette session). Rien de commité.

**Prochaine étape suggérée** : l'utilisateur prévoit de vider les données réelles (Firestore + comptes Auth) pour repartir sur des tests propres — voir la conversation pour les précautions à prendre (platformAdmins protégé en écriture, comptes Auth distincts de Firestore, Cloudinary séparé). Sinon, Module 3 (Stock) reste la prochaine brique logique côté fonctionnalités, ou la collection `CategoryTag` pour débloquer la page Marché.

### 2026-09-25 — Émulateurs Firebase locaux, pour débloquer les tests QA commandes/création de boutique

Signalé par l'utilisateur : impossible de continuer les tests QA sur les commandes et la création de boutique "en attendant les vrais API". **Cause identifiée** : `FIREBASE_SERVICE_ACCOUNT_KEY_BASE64` (compte de service) n'est pas renseigné dans `.env.local` — `getAdminDb()` lève immédiatement, ce qui bloque *toutes* les Server Actions privilégiées introduites ces derniers jours : `createShopAction` (déjà connu, voir l'entrée "assistant Créer ma boutique" du 2026-09-25), et maintenant aussi `createOrderAction`/`updateOrderStatusAction` (Module 4, cette même journée).

Deux options considérées : (a) obtenir un vrai compte de service pour le projet Firebase réel — simple mais nécessite une action manuelle de l'utilisateur dans la console Firebase, pas quelque chose que je peux faire à sa place ; (b) la suite d'émulateurs Firebase (Firestore + Auth), qui ne nécessite aucun identifiant réel et teste le comportement réel du code (règles comprises) en local. **Retenu : (b)**, plus directement actionnable maintenant et sans risque pour le vrai projet Firebase.

**Fait :**
- `firebase.json` : bloc `emulators` (Auth :9099, Firestore :8080, UI :4000). `npm run emulators` (`npx firebase-tools emulators:start`, déjà utilisable via `npx` depuis la Phase 0, rien à installer).
- `src/lib/firebaseAdmin.ts` : si `FIRESTORE_EMULATOR_HOST` est présent, `getAdminApp()` s'initialise sans compte de service — le SDK Admin route alors vers l'émulateur. Nouvel export `getAdminAuth()`.
- `src/lib/verifyIdToken.ts` : **piège réel trouvé en testant** — corriger uniquement `getAdminDb()` ne suffisait pas : `requireCaller()` (utilisé par toutes les Server Actions) vérifie l'ID token via `jose` contre les clés de production Google, qui rejettent *toujours* un token émis par l'émulateur Auth, indépendamment de `FIRESTORE_EMULATOR_HOST`. Corrigé en délégant à `getAdminAuth().verifyIdToken()` (qui sait vérifier un token émulateur) quand `FIREBASE_AUTH_EMULATOR_HOST` est présent. Les deux variables sont donc nécessaires ensemble.
- `src/lib/firebase.ts` (SDK client) : connecte `auth`/`db`/`getSecondaryAuth()` aux émulateurs quand `NEXT_PUBLIC_USE_FIREBASE_EMULATOR === "true"`, protégé contre le double-appel (hot reload).
- `.env.example` documenté ; **`.env.local` de l'utilisateur activé** (`NEXT_PUBLIC_USE_FIREBASE_EMULATOR=true`, `FIRESTORE_EMULATOR_HOST=127.0.0.1:8080`, `FIREBASE_AUTH_EMULATOR_HOST=127.0.0.1:9099`) — repasser ces 3 lignes à vide pour retravailler contre le vrai projet. `.gitignore` : `.firebase/`, `*-debug.log`.

**Vérifié en conditions réelles, pas seulement unitaire** : émulateurs lancés en arrière-plan (`npm run emulators`), les 3 ports répondent (200), script Node ad hoc utilisant `firebase-admin` exactement comme le fait le code de l'app — écriture/lecture Firestore round-trip réussie sans identifiant, `verifyIdToken` atteint bien l'émulateur Auth (rejette proprement un faux token, pas une erreur réseau/JWKS). Émulateurs laissés actifs pour la suite de la session de tests QA de l'utilisateur.

Tests : `verifyIdToken.test.ts` (+2 cas — délégation à l'émulateur, token invalide rejeté proprement), 100% de couverture maintenue.

**Limite assumée, hors scope** : Google/Facebook (vraie authentification OAuth) ne sont pas praticables via l'émulateur Auth de la même façon qu'en production ; email/mot de passe suffit pour les scénarios commandes/création de boutique visés. La notification WhatsApp Business (entrée précédente) reste non fonctionnelle sans ses propres identifiants Meta — comportement déjà silencieux/non bloquant, inchangé.

Vérifié : `npm run lint`, `npx tsc --noEmit`, `npm run build` (toujours 30 routes) et `npm run test:coverage` (324 tests, +2, aucune régression). Rien de commité.

**Pour l'utilisateur** : `npm run emulators` dans un terminal séparé (déjà lancé pour cette session), puis redémarrer `next dev` s'il tournait déjà (les variables d'environnement ne sont lues qu'au démarrage). Base vide à chaque redémarrage de l'émulateur — créer un compte normalement depuis l'app (`role: 'client'` comme en production), passer admin via l'assistant "Créer ma boutique" ou en ajoutant un document `platformAdmins` à la main depuis `http://127.0.0.1:4000/firestore`.

### 2026-09-25 — URL publique par boutique + bouton de partage (BF-64 ciblé, BF-91)

Question posée par l'utilisateur : comment un commerçant partage-t-il le lien de sa boutique publiée sans devoir l'ouvrir et copier l'URL. **Réponse honnête avant de coder** : impossible aujourd'hui — vérifié par exploration que `/catalogue` reste mono-tenant (`ShopService.getPrimaryShop()` renvoie "la première boutique trouvée dans Firestore", sans aucun lien avec le visiteur), et que `Shop.publicToken` (posé pour BF-64 le 2026-09-21) n'est lu/écrit nulle part — un placeholder pur. BF-64 (routage complet `/{tokenOpaque}/{page}/{composant}`) est documenté "non commencé", avec un point ouvert jamais tranché (comment encoder le token).

**Décisions actées avec l'utilisateur avant de coder** : construire une version ciblée maintenant plutôt que la migration complète — nouvelle route `/boutique/{shopId}` utilisant l'id Firestore tel quel (déjà une chaîne non séquentielle, pas besoin d'encoder `ownerId`+`shopId` comme envisagé initialement), en gardant `/catalogue` inchangé.

**Fait :**
- `/boutique/[shopId]` (`src/app/(storefront)/boutique/[shopId]/page.tsx`) : réutilise `CataloguePageContent` telle quelle (déjà shop-agnostique, prend déjà un `shopId` en prop) — résout la boutique via `ShopService.getShop()` (déjà existant), affiche un état honnête "Boutique introuvable ou non publiée" si absente/dépubliée, **sans redirection silencieuse vers `/demo-catalogue`** (contrairement à `/catalogue`) : un lien partagé cassé doit le dire clairement, pas rediriger vers une boutique sans rapport.
- `ShareShopLinkButton` : copier le lien (presse-papiers), lien direct WhatsApp, lien direct email, partage natif (`navigator.share`, détecté dans un effet pour éviter un mismatch d'hydratation — même précaution que `ScrollReveal`/`usePwaInstall`). Utilise `NEXT_PUBLIC_APP_URL`, déclarée depuis la Phase 0 mais jamais consommée jusqu'ici. Affiché dans `ShopSettingsForm` (section Visibilité) et `ShopManagementPageContent` (par carte), uniquement quand la boutique est réellement publiée.

**Piège trouvé en écrivant les tests, documenté pour la suite** : appeler `userEvent.setup()` dans un test de ce composant — même sans jamais utiliser l'instance retournée — empêche silencieusement les `fireEvent.click` suivants d'atteindre les gestionnaires `onClick` du menu déroulant (pas d'erreur, juste aucun effet). Isolé après plusieurs scripts de débogage ; cause exacte non identifiée avec certitude (probablement lié au menu `absolute` par-dessus l'overlay `fixed inset-0` qui le referme au clic extérieur, combiné à l'absence de moteur de layout dans jsdom). Contournement retenu : test entièrement en `fireEvent`, sans `userEvent` du tout — fonctionne de bout en bout, vérifié. `AccountMenu`/`DashboardTopbar` utilisent le même pattern de menu et n'ont jamais été testés jusqu'ici — à garder en tête si une future session leur écrit des tests.
